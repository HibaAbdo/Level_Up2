...
function RoundsButton({ players }) {
  ...
  const submitResultsToBackend = async () => {
    const tournamentId = localStorage.getItem('tournamentId');
    if (!tournamentId) {
      alert('لم يتم العثور على معرف البطولة!');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('فشل إرسال النتائج إلى الخادم');
      }
  ...
  const handleSubmitResults = () => {
    ...
    setResultsSubmitted(false); // لإعادة تفعيل زر "إنهاء"
  };
...
const handleSetResult = (roundIdx, matchId, value) => {
  ...
  if (value === "" || value === "حذف") {
    ...
    // تقسيم النتيجة مثل "0.5-0.5" أو "1-0"
  ...
  // 🟣 حفظ الجولات تلقائيًا في localStorage بعد كل تحديث
useEffect(() => {
...
/>
  ...
  <button className="rounds-action-btn" onClick={() => setIsZoomModalOpen(true)}>
    🔍
  </button>

  <button className="rounds-action-btn" onClick={() => setIsAllRoundsModalOpen(true)}>
    كل الجولات
  </button>
