...
function RoundsPage() {
  ...
  const handleDownloadCSV = () => {
    ...
    let csvContent = "data:text/csv;charset=utf-8,#,اللاعب الأبيض,نقاط,النتيجة,نقاط,اللاعب الأسود\n";
    round.matches.forEach((match, i) => {
      csvContent += `${i + 1},${match.white},${match.whitePts},${match.result},${match.blackPts},${match.black}\n`;
    });
        ...
        {round.matches.length > 0 ? (
          round.matches.map((match, idx) => (
            <tr key={idx}>
              ...
              <td>
                {roundIdx === rounds.length - 1 && match.result !== "Bye" ? (
                  <select
                    value={match.result}
                    onChange={(e) => handleSetResult(roundIdx, idx, e.target.value)}
                  >
